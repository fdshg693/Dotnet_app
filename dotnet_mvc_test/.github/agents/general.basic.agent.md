...
tools: ['execute/getTerminalOutput', 'execute/runTask', 'execute/getTaskOutput', 'execute/createAndRunTask', 'execute/runInTerminal', 'read/problems', 'read/readFile', 'read/terminalSelection', 'read/terminalLastCommand', 'edit', 'search', 'web/fetch', 'agent', 'todo']
---
<context>
...
<step number="2" name="プロジェクトコンテキスト">
    プロジェクト構造の理解が有益な場合、以下から必要なファイルを読み込んでください：
    `.ai\knowledge\general`
...
<step number="5" name="検証">
    ...
    検証内容：
    - #tool:agent/runSubagentでaction_checkエージェントを使用して、ユーザの意図に沿って正しく実装されているか確認
...
<step_5>
    #tool:agent/runSubagent task="ユーザの意図に沿って正しく実装されているか確認" agent="action_check" context="${project_root}"
    結果：認証機能はユーザの要件を満たしています。
