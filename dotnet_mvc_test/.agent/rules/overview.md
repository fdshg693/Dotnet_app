...
## Project Overview
ASP.NET Core 9.0 MVC blog application with SQLite database, ASP.NET Identity authentication (including 2FA), and admin article management. Uses **Repository Pattern** and **Service Layer**. Japanese-language UI with English code.

## Architecture

### Layers & Data Flow
```
Controllers → Services (interfaces) → Repositories (interfaces) → ApplicationDbContext → SQLite (blog.db)
     ↓
ViewModels → Razor Views
```

- **Services**: Business logic via interface-based DI (e.g., `IArticleService` → `ArticleService`)
- **Repositories**: Data access abstraction (e.g., `IArticleRepository` → `ArticleRepository`)
...
- `Comment` → `Article` (many-to-one), with `IsApproved` flag for moderation
...
- **Two-Factor Authentication (2FA)**: Supported via `ITwoFactorService` / QRCoder. Admins may require 2FA via `[RequireAdminTwoFactor]` attribute.
...
| 2FA & Auth | `Services/TwoFactorService.cs`, `Filters/RequireAdminTwoFactorAttribute.cs` |
...
- Public Articles: `/articles` (Index), `/articles/{slug}` (Details)
- Admin Articles: `/admin/articles`, `/admin/articles/create`, `/admin/articles/{id}`
